...
export function SimulationGraphs() {
  ...
  const chartData = useMemo(() => {
    ...
    return history.snapshots.map((snap, i) => {
      const rawTime = snap.time !== undefined ? snap.time : (history.timestamps ? history.timestamps[i] : 0);
      const timeLabel = (Number(rawTime) / 3600).toFixed(1);

      const dataPoint: any = {
        time: isNaN(Number(timeLabel)) ? i : timeLabel,
        current: parseFloat(getValue(snap, selectedFeatureId, metric).toFixed(2))
      };

      // Merge: Saved Scenarios
      scenarios.forEach((scen) => {
          if (scen.isVisible && scen.results.snapshots[i]) {
            const val = getValue(scen.results.snapshots[i], selectedFeatureId, metric);
